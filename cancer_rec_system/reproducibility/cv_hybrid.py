#!/usr/bin/env python3
"""Resumable historical CV replay for the mutation-aware Hybrid-NCF model.

This is the released, source-faithful port of the private audit runner. It
regenerates the manuscript's cross-validated RMSE for Hybrid-NCF from the exact
private training snapshot, the mutation target table, and the maintained
mutation feature builder. The model classes, ``build_ncf``, ``load_pretrained``
transfer, hyperparameter sampler, per-stage training loop, fold-model
evaluation, and the tuning orchestration are copied verbatim from the audited
historical source so the torch/NumPy RNG consumption order is preserved.

The mutation side features are built by ``training/features.py``. Its pure
feature-building functions were validated against both the historical and
current ``mutation.py`` implementations and reproduce the audited mappings and
feature tensors exactly while accepting explicit input paths.
...
The runner requires the private training snapshot and the mutation target
table; it needs no model weights, mappings, or recommendation CSVs.
"""
...
from ..training.features import build_feature_frames, build_intervention_vectors
from .common import read_json, sha256_file, verify_csv
...
DEFAULT_TARGETED = PRIVATE_DATA / "targeted_therapy.csv"
RUNS_DIR = APP_ROOT / "params" / "cv-hybrid-runs"
CONFIG_PATH = HERE / "configs" / "cv_hybrid.json"
FEATURE_BUILDER = APP_ROOT / "training" / "features.py"

SEED = 2000
# Latest manuscript value; the earlier 0.433 claim remains documented in the
# reference config for audit history.
PAPER_RMSE = 0.4643
...
EXPECTED_ROWS = 32960
EXPECTED_CANCERS = 55
EXPECTED_ITEMS = 12049
EXPECTED_FEATURES = 31
EXPECTED_PREPARED_FINGERPRINTS = {
    "user_ids_sha256": (
        "eb94cef3bd47ba5020207f874e352d2c1bc5453f8da34f24fb96a2cc1f256bec"
    ),
    "item_ids_sha256": (
        "c208115ba002d9740550087971a6430462188023f26acc96facc53053acc3804"
    ),
    "labels_float64_sha256": (
        "8a2686794515e2d65478dfd3b8d1f3b7a575199b8f7f46045809b1b1779cc754"
    ),
    "pairs_sha256": (
        "c3791b6061ea200e2512db8769a9b5b654d9b2c56586f60852b034c0d9ca2db1"
    ),
    "user_mapping_sha256": (
        "1c488a035316559616a93598453c9cdbd6ee9a9051dc524d465972159db3cdef"
    ),
    "item_mapping_sha256": (
        "a7bfc66d771d3775caa3d4a3ae5058e9b845deb49da8640bfe0c2c0b878f9c2a"
    ),
    "user_features_float32_sha256": (
        "5c3c3217fcf6a91f215b1cc331553f0977d3a2d430ac0eeec3bab838c92c820b"
    ),
    "item_features_float32_sha256": (
        "d845a6bf611c30345d70c407d64751f8aae832ab9f3b225b0a3bad288d2a99d7"
    ),
}
...
def input_hashes_for(paths: Sequence[Path]) -> Dict[str, str]:
...
def ordered_names(mapping: Mapping[str, int]) -> List[str]:
    inverse = {int(value): str(key) for key, value in mapping.items()}
    if set(inverse) != set(range(len(inverse))):
        raise ValueError("Mappings must use contiguous zero-based IDs.")
    return [inverse[index] for index in range(len(inverse))]


class HybridGMF(nn.Module):
    def __init__(self, num_users: int, num_items: int, embedding_dim: int,
                 user_feature_dim: int, item_feature_dim: int, gmf_dim: int):
        super().__init__()
    ...
    def forward(self, user_input: torch.Tensor, item_input: torch.Tensor,
        ...
        output = self.gmf_user_proj(user_vec) * self.gmf_item_proj(item_vec)
        return self.output_layer(output).view(-1)


class HybridMLP(nn.Module):
    def __init__(self, num_users: int, num_items: int, embedding_dim: int,
                 user_feature_dim: int, item_feature_dim: int,
                 hidden_units: Sequence[int], dropout_rate: float):
        super().__init__()
        self.user_embedding = nn.Embedding(num_users, embedding_dim)
        self.item_embedding = nn.Embedding(num_items, embedding_dim)
        ...
        self.output_layer = nn.Linear(int(hidden_units[-1]), 1)

    def forward(self, user_input: torch.Tensor, item_input: torch.Tensor,
                user_features: torch.Tensor, item_features: torch.Tensor) -> torch.Tensor:
        user_vec = torch.cat([self.user_embedding(user_input), user_features], dim=-1)
        item_vec = torch.cat([self.item_embedding(item_input), item_features], dim=-1)
        return self.output_layer(self.mlp_layers(torch.cat([user_vec, item_vec], dim=-1))).view(-1)
...
def build_ncf(hp: Mapping[str, Any], num_users: int, num_items: int,
              user_feature_dim: int, item_feature_dim: int) -> HybridNCF:
    return HybridNCF(
        num_users, num_items, int(hp["embedding_dim"]), user_feature_dim,
        item_feature_dim, int(hp["gmf_dim"]),
        [int(value) for value in hp["hidden_units"]], float(hp["dropout_rate"]),
    )


def load_pretrained(model: HybridNCF, gmf: Mapping[str, torch.Tensor],
                    mlp: Mapping[str, torch.Tensor],
                    alpha: float = PRETRAINED_ALPHA) -> HybridNCF:
    state = model.state_dict()
    for name in ("gmf_user_proj.weight", "gmf_user_proj.bias",
                 "gmf_item_proj.weight", "gmf_item_proj.bias"):
        state[name] = gmf[name]
    for name in list(state):
        if "mlp_layers" in name:
            state[name] = mlp[name]
    state["user_embedding.weight"] = (
        gmf["user_embedding.weight"] + mlp["user_embedding.weight"]
    ) / 2.0
    state["item_embedding.weight"] = (
        gmf["item_embedding.weight"] + mlp["item_embedding.weight"]
    ) / 2.0
    state["output_layer.weight"] = alpha * torch.cat(
        [gmf["output_layer.weight"], mlp["output_layer.weight"]], dim=1
    )
    state["output_layer.bias"] = alpha * (
        gmf["output_layer.bias"] + mlp["output_layer.bias"]
    )
...
class Prepared:
    ...
    row_user_features: torch.Tensor
    row_item_features: torch.Tensor
    feature_builder_file: str
    feature_builder_hash: str


def prepared_fingerprints(prepared: Prepared) -> Dict[str, str]:
    """Fingerprint every order-sensitive prepared input used by training."""
    users = prepared.data["user_id"].to_numpy(dtype=np.int64)
    items = prepared.data["item_id"].to_numpy(dtype=np.int64)
    labels = prepared.labels.astype(np.float64)
    pairs = prepared.data[["Cancer", "Intervention"]].astype(str)
    return {
        "user_ids_sha256": sha256_bytes(users.tobytes()),
        "item_ids_sha256": sha256_bytes(items.tobytes()),
        "labels_float64_sha256": sha256_bytes(labels.tobytes()),
        "pairs_sha256": sha256_bytes(
            pd.util.hash_pandas_object(pairs, index=False).values.tobytes()
        ),
        "user_mapping_sha256": sha256_bytes(
            pickle.dumps(prepared.user_mapping, protocol=4)
        ),
        "item_mapping_sha256": sha256_bytes(
            pickle.dumps(prepared.item_mapping, protocol=4)
        ),
        "user_features_float32_sha256": sha256_bytes(
            prepared.user_features.numpy().tobytes()
        ),
        "item_features_float32_sha256": sha256_bytes(
            prepared.item_features.numpy().tobytes()
        ),
    }


def prepare_data(
    summary_path: Path, targeted_path: Path, allow_unverified: bool
) -> Prepared:
    data = pd.read_csv(summary_path)
    targeted = pd.read_csv(targeted_path)
    data.reset_index(drop=True, inplace=True)
    data = data.dropna(subset=["Cancer", "Intervention"]).copy()
    data["intervention_id"] = data["Intervention"].astype("category").cat.codes + 1
    ...
    counts = data.groupby("Cancer")["Intervention"].count()
    data = data[data["Cancer"].isin(counts[counts >= 20].index)].copy().reset_index(drop=True)
    data["Intervention"] = data["Intervention"].map(canonical_intervention)
    user_mapping = {str(value): index for index, value in enumerate(data["Cancer"].unique())}
    item_mapping = {str(value): index for index, value in enumerate(data["Intervention"].unique())}
    data["user_id"] = data["Cancer"].map(user_mapping).astype(np.int64)
    data["item_id"] = data["Intervention"].map(item_mapping).astype(np.int64)

    cancer_features, drug_features, _ = build_feature_frames(
        data,
        targeted,
    )
    intervention_features = build_intervention_vectors(
        item_mapping,
        drug_features,
    )
    user_names = ordered_names(user_mapping)
    item_names = ordered_names(item_mapping)
    user_features = torch.tensor(
        cancer_features.loc[user_names].to_numpy(dtype=np.float32), dtype=torch.float32
    )
    item_features = torch.tensor(
        intervention_features.loc[item_names].to_numpy(dtype=np.float32), dtype=torch.float32
    )
    row_users = torch.tensor(data["user_id"].to_numpy(dtype=np.int64), dtype=torch.long)
    row_items = torch.tensor(data["item_id"].to_numpy(dtype=np.int64), dtype=torch.long)
    prepared = Prepared(
        data=data,
        ...
        row_user_features=user_features[row_users],
        row_item_features=item_features[row_items],
        feature_builder_file=str(FEATURE_BUILDER.resolve()),
        feature_builder_hash=sha256_file(FEATURE_BUILDER),
    )
    observed = (len(data), len(user_mapping), len(item_mapping), user_features.shape[1])
    expected = (EXPECTED_ROWS, EXPECTED_CANCERS, EXPECTED_ITEMS, EXPECTED_FEATURES)
    if observed != expected and not allow_unverified:
        raise RuntimeError(
            f"Preprocessing dimensions changed: observed={observed}, "
            f"expected={expected}"
        )
    fingerprints = prepared_fingerprints(prepared)
    mismatches = {
        name: {
            "expected": expected_hash,
            "observed": fingerprints.get(name),
        }
        for name, expected_hash in EXPECTED_PREPARED_FINGERPRINTS.items()
        if fingerprints.get(name) != expected_hash
    }
    if mismatches and not allow_unverified:
        raise RuntimeError(
            "Prepared data or feature fingerprints changed: "
            + json.dumps(mismatches, sort_keys=True)
        )
    return prepared


def create_row_splits(
    data: pd.DataFrame, folds: int
) -> List[Tuple[np.ndarray, np.ndarray]]:
    return list(
        KFold(n_splits=folds, shuffle=True, random_state=SEED).split(
...
def sample_hyperparameters() -> Dict[str, object]:
    ...
    hidden_units = sorted(
        [int(np.random.choice([64, 128, 256])) for _ in range(number_hidden)],
        reverse=True,
    )
    return {
        ...
        "weight_decay": float(10 ** np.random.uniform(-6, -2)),
    }
...
def train_stage(
    ...
    """Literal legacy batch-mean validation and early-stopping behavior."""
    model.cpu()
    ...
    for epoch in range(epochs):
        ...
        train_sse = 0.0
        train_n = 0
        for user, item, user_features, item_features, rating in train_loader:
            optimizer.zero_grad()
            output = model(user, item, user_features, item_features)
            loss = criterion(output, rating)
            loss.backward()
            optimizer.step()
            train_sse += float(torch.sum((output.detach() - rating) ** 2))
            train_n += len(rating)
        model.eval()
        ...
        with torch.no_grad():
            for user, item, user_features, item_features, rating in validation_loader:
                output = model(user, item, user_features, item_features)
                validation_batch_sum += float(criterion(output, rating))
                validation_sse += float(torch.sum((output - rating) ** 2))
                validation_n += len(rating)
                validation_batches += 1
        legacy_loss = validation_batch_sum / validation_batches
        scheduler.step(legacy_loss)
        improved = legacy_loss < best_loss
        if improved:
            best_loss = legacy_loss
        ...
        history.append({
            "epoch": epoch + 1,
            "train_sample_weighted_mse": train_sse / train_n,
            "validation_legacy_unweighted_batch_mse": legacy_loss,
            "validation_sample_weighted_mse": validation_sse / validation_n,
    ...
    if best_weights is None:
        raise RuntimeError("Training stage produced no best weights.")
    return best_weights, best_loss, best_epoch, epoch + 1, history


def make_fold_datasets(
    prepared: Prepared, train_index: np.ndarray, validation_index: np.ndarray
) -> Tuple[TensorDataset, TensorDataset]:
    users = torch.tensor(prepared.data["user_id"].to_numpy(dtype=np.int64), dtype=torch.long)
    items = torch.tensor(prepared.data["item_id"].to_numpy(dtype=np.int64), dtype=torch.long)
    labels = torch.tensor(prepared.labels.astype(np.float32), dtype=torch.float32)
    train = torch.tensor(train_index, dtype=torch.long)
    validation = torch.tensor(validation_index, dtype=torch.long)
    train_dataset = TensorDataset(
        users[train], items[train], prepared.row_user_features[train],
        prepared.row_item_features[train], labels[train],
    )
    validation_dataset = TensorDataset(
        users[validation], items[validation], prepared.row_user_features[validation],
        prepared.row_item_features[validation], labels[validation],
    )
...
def run_trial(
    ...
    gmf_state, gmf_loss, gmf_best, gmf_run, gmf_history = train_stage(
        gmf, train_loader, validation_loader, learning_rate,
        pretrain_epochs, pretrain_patience, weight_decay,
    )
    mlp = HybridMLP(
        **common,
    ...
    mlp_state, mlp_loss, mlp_best, mlp_run, mlp_history = train_stage(
        mlp, train_loader, validation_loader, learning_rate,
        pretrain_epochs, pretrain_patience, weight_decay,
    )
    ncf = build_ncf(
        hp, len(prepared.user_mapping), len(prepared.item_mapping),
        prepared.user_features.shape[1], prepared.item_features.shape[1],
    )
    ncf = load_pretrained(ncf, gmf_state, mlp_state)
    ncf_state, ncf_loss, ncf_best, ncf_run, ncf_history = train_stage(
        ncf, train_loader, validation_loader, learning_rate * 0.1,
        finetune_epochs, finetune_patience, weight_decay,
    )
    record = {
        "hyperparameters": dict(hp),
        "gmf": {"best_legacy_mse": gmf_loss, "best_epoch": gmf_best,
                "epochs_run": gmf_run, "history": gmf_history},
        "mlp": {"best_legacy_mse": mlp_loss, "best_epoch": mlp_best,
                "epochs_run": mlp_run, "history": mlp_history},
        "hybrid": {"best_legacy_mse": ncf_loss, "best_epoch": ncf_best,
                   "epochs_run": ncf_run, "history": ncf_history},
    }
    del gmf, mlp, ncf, train_loader, validation_loader
    gc.collect()
    return ncf_state, ncf_loss, record


def evaluate_fold_model(
    weights: Mapping[str, torch.Tensor], hp: Mapping[str, Any], prepared: Prepared,
    validation_dataset: TensorDataset, validation_labels: np.ndarray,
) -> Tuple[np.ndarray, Dict[str, float]]:
    # Historical RandomSearch constructs the returned best model here.  This
    # constructor's RNG consumption is part of the next trial's identity.
    model = build_ncf(
        hp, len(prepared.user_mapping), len(prepared.item_mapping),
        prepared.user_features.shape[1], prepared.item_features.shape[1],
    )
    ...
    model.cpu().eval()
    loader = DataLoader(validation_dataset, batch_size=1024)
    predictions: List[float] = []
    with torch.no_grad():
        for user, item, user_features, item_features, _rating in loader:
            predictions.extend(model(user, item, user_features, item_features).tolist())
...
def final_summary(
    ...
    output = {
        ...
        "input_hashes_end": input_hashes_for(list(input_paths)),
...
def run(args: argparse.Namespace) -> None:
    ...
    summary_raw = pd.read_csv(data_path)
    summary_record = verify_csv(
        data_path,
        summary_raw,
        "training_snapshot",
        allow_unverified=args.allow_unverified_inputs,
    )
    targeted_raw = pd.read_csv(targeted_path)
    targeted_record = verify_csv(
        targeted_path,
        targeted_raw,
        "targeted_therapy",
        allow_unverified=args.allow_unverified_inputs,
    )
    input_paths = [data_path, targeted_path]
    input_hashes = input_hashes_for(input_paths)
    ...
    prepared = prepare_data(
        data_path, targeted_path, args.allow_unverified_inputs
    )
    prepared_hashes = prepared_fingerprints(prepared)
    prepared_hash_checks = {
        name: prepared_hashes.get(name) == expected_hash
        for name, expected_hash in EXPECTED_PREPARED_FINGERPRINTS.items()
    }
    labels = prepared.labels
    if args.protocol == "faithful":
        splits = create_row_splits(prepared.data, args.folds)
    else:
        splits = create_pair_grouped_splits(prepared.data, args.folds)
    diagnostics = split_diagnostics(prepared.data, splits)
    config = {
        "seed": SEED,
        "folds": args.folds,
        ...
        "device": "cpu",
        "features": int(prepared.user_features.shape[1]),
        "split": (
            "row_kfold"
            if args.protocol == "faithful"
    ...
    run_identity = {
        ...
        "prepared_fingerprints": prepared_hashes,
    ...
    manifest = {
        ...
        "configuration": config,
        "data": {
            "rows": len(prepared.data),
            "cancers": len(prepared.user_mapping),
            "items": len(prepared.item_mapping),
            "features": int(prepared.user_features.shape[1]),
        },
        "training_input": summary_record,
        "targeted_input": targeted_record,
        "input_hashes": input_hashes,
        "feature_builder": {
            "path": prepared.feature_builder_file,
            "sha256": prepared.feature_builder_hash,
            "module": "cancer_rec_system.training.features",
        },
        "prepared_fingerprints": {
            "actual": prepared_hashes,
            "checks": prepared_hash_checks,
            "all_exact": all(prepared_hash_checks.values()),
        },
        "split_diagnostics": diagnostics,
        "environment": execution_environment,
    ...
    for fold_index in range(int(state["next_fold"]), args.folds):
        ...
        train_dataset, validation_dataset = make_fold_datasets(
            prepared, train_index, validation_index
        )
        ...
        prediction_frame = pd.DataFrame(
            {
                ...
                "cancer": prepared.data.iloc[validation_index]["Cancer"].to_numpy(),
                "intervention": prepared.data.iloc[validation_index]["Intervention"].to_numpy(),
                "label": validation_labels,
                "prediction": predictions,
                "squared_error": (predictions - validation_labels) ** 2,
            }
        )
        prediction_path = (
            run_dir
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Resumable historical CV replay for the mutation-aware Hybrid-NCF "
            "model. The faithful protocol regenerates the reported manuscript "
            "RMSE."
        )
    )
    parser.add_argument(
